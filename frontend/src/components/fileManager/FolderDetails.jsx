...
import FolderEditPopup from "../UI/PopUp/ProjectManagemnet/FolderEditPopup";
import PageTitle from "../page-header/PageHeader";
import AddFolder from "./AddFolder";

const FolderDetails = () => {
    const { id } = useParams();
    const { data: folderData, isLoading: folderDataLoading } =
        useGetFoldersQuery({ parentId: id });
    const { data: folderDirectoryData, isLoading: folderDirectoryDataLoading } =
        useGetFolderDirectoryQuery(id);
    const { data, isLoading, refetch } = useGetFileByFolderIdQuery(id);
        ...
        <div className="relative">
            <PageTitle
                ...
                extra={
                    <>
                        {" "}
                        <CommonDelete
                            permission={"delete-fileManager"}
                            deleteThunk={
                                fileManagerApi.endpoints.deleteFolder.initiate
                            }
                            id={id}
                        />
                        <FolderEditPopup
                            data={folderDirectoryData && folderDirectoryData}
                        />
                    </>
                }
            />
            <div className=" bg-white rounded-lg mt-2 pb-20 flex flex-col gap-2">
                <div className="bg-zinc-600 text-white mb-4 px-4 py-1 rounded-sm ">
                    {folderDirectoryData &&
                        !folderDirectoryDataLoading &&
                        `${folderDirectoryData.folderDirectory}${folderDirectoryData.folderName}`}
                </div>
                                ...
                                >
                                    ...
                                    <div className="mt-2 ml-auto flex gap-2">
                                        <CommonDelete
                                            ...
                                            id={file.id}
                                            navigatePath={`/admin/folderDetails/${id}`}
                                        />
                                        <button
